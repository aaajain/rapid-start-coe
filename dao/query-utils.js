	...
	createAdminUser: function(callback){
		...
		conn.collection("role_masters").find({ role_name: 'admin' }).toArray(function(err, result) {
	    	...
	    	else if(result && result.length > 0)
	    	{
	    		...
	    		admin_user.findOneAndUpdate({username:'admin'},{$set: { email: 'admin@admin.com',username:'admin',password:hash,role:roleId}},{upsert: true, new: true, runValidators: true},function (err,doc) {
					 if(err)
					 {
					 	logger.debug(err.stack);
					 	callback(err,null);
